...
def set_work_dir(local_path="ner_bert_crf", server_path="ner_bert_crf"):
    path_base = "/home/ducnh/PycharmProjects/195/NER-BERT-CRF"
...
output_dir = '/home/ducnh/PycharmProjects/195/NER-BERT-CRF/output'
...
def evaluate(model, predict_dataloader, batch_size, epoch_th, dataset_name):
    print("***** Running prediction ***** 1111111111111111111111111111111")
    model.eval()
    all_preds = []
    all_labels = []
    total=0
...
#         if all_preds[i] == all_preds[i]:
#             print(i)
#             print(predict_dataloader[i])
#             print('^^^^')
#     return test_acc, f1

def evaluate(model, predict_dataloader, batch_size, epoch_th, dataset_name):
    print("***** Running prediction 777777777777777777*****")
    model.eval()
    all_preds = []
    all_labels = []
    total = 0
    ...
    with torch.no_grad():
        for batch in predict_dataloader:
            ...
            #     if valid_predicted[i] != valid_label_ids[i]:
            #         misclassified_count += 1
            #         break
            #         mispredicted_words.append(predict_dataloader.dataset[i])
    ...
    print('predict bert + crf---------------------------------------------')
    # print(all_preds)
    print('labels bert + crf---------------------------------------------')
    # print(all_labels)
    print('chinh xac bert + crf ++++++++++++++++++++++++++++++++++')
    print(correct)
    acc = []
    # print('Misclassified Words:')
    # for word in mispredicted_words:
    #     print(word)
    #     print('^^^^')
    # print(mispredicted_words)
    for i in range(len(all_labels)):
        if all_labels != 4:
            acc.append("5")
        elif  all_labels[i] == 4 and all_preds != 4:
            acc.append("1")
        else:
            acc.append("0")
    return test_acc, f1, acc
...
for epoch in range(start_epoch, total_train_epochs):
    ...
    print("Epoch:{} completed, Total training's Loss: {}, Spend: {}m".format(epoch, tr_loss, (time.time() - train_start)/60.0))
    # acc = []
    valid_acc, valid_f1, acc = evaluate(model, dev_dataloader, batch_size, epoch, 'Valid_set')
    print(acc)
...
model.to(device)
#evaluate(model, train_dataloader, batch_size, total_train_epochs-1, 'Train_set')
print('test day nay 822222222222222222')
evaluate(model, test_dataloader, batch_size, epoch, 'Test_set')
...
# import csv
#
# def write_list_to_csv(acc, file_path):
...
#
# write_list_to_csv(my_list, file_path)
valid_acc, valid_f1, acc = evaluate(model, test_dataloader, batch_size, epoch, 'Test_set')
def write_list_to_text(data_list, file_path):
...
my_list = acc
file_path = '/home/ducnh/PycharmProjects/195/2/NER-BERT-CRF/output.txt'

write_list_to_text(my_list, file_path)
